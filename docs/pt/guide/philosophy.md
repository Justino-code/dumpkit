...
O nodedump é construído sobre um princípio simples:
...
- Sem verificações de `NODE_ENV` - tu controlas quando chamar o nodedump
...
| Funcionalidade | console.log | nodedump |
|----------------|-------------|----------|
| Formatação bonita | ❌ | ✅ |
...
O nodedump faz uma coisa e faz bem: **debugging**.
...
Mas podes **combinar** o nodedump com essas ferramentas - o `inspect()` dá-te a string formatada que podes enviar para qualquer lado.